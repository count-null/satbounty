...
impl Context {
    pub async fn raw(
        ...
        let bounty_cards =
            BountyCardDisplay::all_pending_for_user(&mut db, user.id, PAGE_SIZE, page_num)
                .await
...
async fn index(
    ...
    Ok(Template::render("mypendingbounties", context))
}

pub fn my_pending_bounties_stage() -> AdHoc {
    AdHoc::on_ignite("My Pending Bounties Stage", |rocket| async {
        rocket.mount("/my_pending_bounties", routes![index])
