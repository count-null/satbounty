...
impl Context {
    pub async fn raw(
        ...
        let bounty_cards = BountyCardDisplay::all_deactivated(&mut db, PAGE_SIZE, page_num)
            .await
...
async fn index(
    ...
    Ok(Template::render("deactivatedbountiesindex", context))
}

pub fn deactivated_bounties_stage() -> AdHoc {
    AdHoc::on_ignite("Deactivated Bounties Stage", |rocket| async {
        rocket.mount("/deactivated_bounties", routes![index])
    })
}
